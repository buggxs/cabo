...
@GenerateNiceMocks(<MockSpec<dynamic>>[
  ...
  MockSpec<HttpsCallableResult<void>>(as: #MockCallableResult),
])
void main() {
  ...
  group('AuthService mail functions', () {
    late MockitoFirebaseAuth auth;
    late MockitoUser user;
    ...
    setUp(() {
      ...
      service = AuthService(auth: auth, functions: functions);
      when(auth.currentUser).thenReturn(user);
      when(functions.httpsCallable(any)).thenReturn(callable);
    });

    test('sends the verification mail through our own function', () async {
      // Firebase blocks editing this project's templates, so the mail comes
      // from a Cloud Function rather than from sendEmailVerification().
      when(
        callable.call<void>(any),
      ).thenAnswer((_) async => MockCallableResult());

      final AuthOutcome outcome = await service.sendVerificationEmail();

      expect(outcome.isSuccess, isTrue);
      verify(functions.httpsCallable('sendVerificationEmail')).called(1);
      verifyNever(user.sendEmailVerification(any));
    });

    test('reports the throttle as tooManyRequests', () async {
      ...
      expect(outcome.error, AuthError.tooManyRequests);
    });

    test('reports an offline call as a network error', () async {
      when(callable.call<void>(any)).thenThrow(
    ...
    test('fails without a signed in user instead of calling out', () async {
      when(auth.currentUser).thenReturn(null);
    ...
    test('passes the address on for a password reset', () async {
      when(
        callable.call<void>(any),
      ).thenAnswer((_) async => MockCallableResult());

      final AuthOutcome outcome = await service.sendPasswordResetEmail(
        'player@example.com',
      );

      expect(outcome.isSuccess, isTrue);
      verify(functions.httpsCallable('sendPasswordResetEmail')).called(1);
      verify(
        callable.call<void>(<String, dynamic>{'email': 'player@example.com'}),
      ).called(1);
    });
  });
